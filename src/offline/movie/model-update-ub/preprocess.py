...
def gen_data_set(data, negsample=0, item_ids=None):
    data.sort_values("stat_date", inplace=True)
    #     item_ids = data['movie_id'].unique()

    train_set = []
    test_set = []
    for reviewerID, hist in tqdm(data.groupby('u_id')):
        pos_list = hist['c_id'].tolist()
        ...
        for i in range(1, len(pos_list)):
            ...
            if i <= seq_short_len and i != len(pos_list) - 1:
                train_set.append((reviewerID, hist[::-1], [0] * seq_prefer_len, pos_list[i], 1, len(hist[::-1]), 0,
                                  rating_list[i], [0] * seq_prefer_len))
            #                 train_set.append((reviewerID, hist[::-1], [0]*seq_prefer_len, pos_list[i], 1, len(hist[::-1]), 0,
            #                                   rating_list[i], genres_hist[::-1], [0]*seq_prefer_len))
                ...
                train_set.append(
                    (reviewerID, hist[::-1][:seq_short_len], hist[::-1][seq_short_len:], pos_list[i], 1, seq_short_len,
                     len(hist[::-1]) - seq_short_len, rating_list[i]))
...
def gen_model_input(train_set, user_profile, seq_max_len):
    ...
    train_model_input = {"u_id": train_uid, "c_id": train_iid, "hist_card_song_id": train_seq_pad,
...
def gen_model_input_sdm(train_set, user_profile, seq_short_len, seq_prefer_len):
    ...
    train_model_input = {"u_id": train_uid, "c_id": train_iid, "short_card_song_id_id": train_short_item_pad,
